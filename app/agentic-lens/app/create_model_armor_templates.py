...
def main():
    ...
    res, err = make_post_request(medium_url, medium_payload, headers)
    if err:
        code, body = err
        if "already exists" in body.lower() or "already_exists" in body.lower() or "already in use" in body.lower() or code == 409:
            print("  Template security-medium already exists/in-use; skipping.")
        else:
    ...
    res, err = make_post_request(high_url, high_payload, headers)
    if err:
        code, body = err
        if "already exists" in body.lower() or "already_exists" in body.lower() or "already in use" in body.lower() or code == 409:
            print("  Template security-high already exists/in-use; skipping.")
        else:
