...
XRAY_MANAGER_PEERS = (
    "AGENTIC_LENS_ENGINE_XRAY_LIBRARIAN",
    "AGENTIC_LENS_ENGINE_XRAY_ARCHITECT",
    "AGENTIC_LENS_ENGINE_XRAY_SPECIALIST",
    "AGENTIC_LENS_ENGINE_XRAY_AUDITOR",
)
...
def main() -> int:
    ...
    for name in OPTIONAL_AGENTS:
        ...
        elif name == "xray_manager":
            for pk in XRAY_MANAGER_PEERS:
                if not (ev.get(pk) or "").strip():
                    errs.append(f"xray_manager: missing {pk} (run ./deploy.sh after merge_peer_engine_env.py)")
    if errs:
