...
import ssl
# Disable SSL verification for self-signed load balancer certificates during local testing
...
def _fetch_iap_token(audience: str) -> str:
    # Allow manual override / pre-fetched token
    env_token = os.environ.get("IAP_OAUTH_TOKEN")
    if env_token:
        return env_token
    ...
    try:
        ...
        print(f"Warning: direct token refresh failed: {e}", file=sys.stderr)
...
def main() -> None:
    ...
    def _health() -> None:
        url = f"{base}/api/healthz?deep=1"
        print(f"Diagnostic: url={url} headers={list(headers.keys())}")
