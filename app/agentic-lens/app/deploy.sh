...
echo "Ensuring identity_type=AGENT_IDENTITY, deployment config, tracing env_vars, dual-gateways, and per-department CMEK..."
...
for agent in "${AGENTS[@]}"; do
  ...
  if [[ -d "$agent_dir" ]]; then
    ...
    ingress_gw="projects/${PROJECT_ID}/locations/${REGION}/agentGateways/main-ingress-agw"
    egress_gw="projects/${PROJECT_ID}/locations/${REGION}/agentGateways/main-egress-agw-v2"

    python3 -c '
import json, sys
path = sys.argv[1]
deploy_cfg = json.loads(sys.argv[2])
kms_key = (sys.argv[3] or "").strip()
ingress_gw = sys.argv[4].strip()
egress_gw = sys.argv[5].strip()
try:
...
c["env_vars"] = {
  **(c.get("env_vars") or {}),
  ...
  "PYTHONHTTPSVERIFY": "0",
  "PYTHONPATH": f"/code/{sys.argv[6].strip()}_staged",
...
c["agent_gateway_config"] = {
  "client_to_agent_config": {"agent_gateway": ingress_gw},
  "agent_to_anywhere_config": {"agent_gateway": egress_gw}
}
if kms_key:
...
' "$config_file" "$DEPLOY_CONFIG" "$kms_key" "$ingress_gw" "$egress_gw" "$agent" 2>/dev/null || true
  fi
done
...
deploy_one() {
  ...
  if [[ "$agent" == "supervisor" ]]; then
    ...
    if [[ -f "$ADK_DIR/sitecustomize.py" ]]; then
      cp -a "$ADK_DIR/sitecustomize.py" "$sup_bundle/ws/"
    fi
    ...
    find "$sup_bundle/ws/agents/supervisor/peer_agents" -name "sitecustomize.py" -delete 2>/dev/null || true
    find "$sup_bundle/ws/agents/supervisor/peer_agents" -name "ca-certificates.crt" -delete 2>/dev/null || true
  ...
  if (cd "$deploy_root" && $adk_wrap "$ADK_CMD" deploy agent_engine --project="$PROJECT_ID" --region="$REGION" --temp_folder="${agent}_staged" $adk_extra "$deploy_relpath" 2>&1) > "$deploy_out"; then
    if grep -qE "Failed to create Agent Engine|failed to start and cannot serve traffic|'code': 13|Please refer to our documentation.*troubleshooting|Deploy failed|not available in region" "$deploy_out" 2>/dev/null && ! grep -qE "Updated agent engine|Created agent engine" "$deploy_out" 2>/dev/null; then
      cat "$deploy_out" >&2
      rm -f "$deploy_out"
