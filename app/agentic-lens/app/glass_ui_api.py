...
# Force socket resolution to IPv4 globally in this process for httpx/urllib/gRPC
try:
    import socket
...
# Force gRPC secure channels to use IPv4 (resolving dns:/// targets to IPv4 IPs and overriding ssl target name)
try:
    import builtins
    ...
    def patched_import(name, *args, **kwargs):
        ...
        if "grpc" in sys.modules:
            g = sys.modules["grpc"]
            if hasattr(g, "secure_channel") and not hasattr(g, "_patched_ipv4"):
                g._patched_ipv4 = True
                orig_secure_channel = g.secure_channel
                def patched_secure_channel(target, credentials, options=None, **kwargs):
                    if "googleapis.com" in target:
                        try:
                            clean_target = target.replace("dns:///", "").replace("dns:", "")
                            if ":" in clean_target:
                                host, port_str = clean_target.split(":", 1)
                                port = int(port_str)
                            else:
                                host = clean_target
                                port = 443
                            ...
                            if infos:
                                ...
                                return orig_secure_channel(new_target, credentials, options=options, **kwargs)
                        except Exception:
                            pass
                    return orig_secure_channel(target, credentials, options=options, **kwargs)
                g.secure_channel = patched_secure_channel
        return module
...
def _execute_query_request(req: QueryRequest) -> QueryResponse:
    ...
    if security_level != "off":
        ...
        if not armor_result.get("ran", False):
            import sys
            sys.stderr.write(f"[MODEL ARMOR ERROR / BYPASS]: {armor_result.get('skip_reason')}\n")
            sys.stderr.flush()
