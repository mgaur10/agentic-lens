...
if (not os.path.exists("/code") and not os.path.exists("/app")) or "peer_agents" in __name__ or ("__file__" in globals() and "peer_agents" in __file__):
    pass
else:
        ...
        def _patched_secure_channel(target, credentials, options=None, compression=None):
            ...
            if "googleapis.com" in target:
                ...
                if not target.startswith("ipv4:"):
                    ...
                    port = port or "443"
                    try:
                        if not os.path.exists("/code") and os.path.exists("/app"):
                            ip = "240.0.0.2"
                        else:
    ...
    try:
        ...
        # 2. Add import interceptor to catch google.adk.runners on load
        from importlib.abc import MetaPathFinder
