...
# ADK tool refs like `src.manager.*` assume this agent root is on sys.path. Required when vendored
# under supervisor as `peer_agents.xray_manager` (container cwd is not the agent directory).
...
class SafeLlmAgent(LlmAgent):
    ...
    def from_config(cls, config, config_abs_path: str):
        ...
        return config_agent_utils.from_config(config_abs_path)
