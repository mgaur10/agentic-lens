...
Exposes orchestrate_build as the single tool; instruction tells the model
to call it with the user query.
"""
...
try:
    ...
    from manager import orchestrate_build

LEAD_INSTRUCTION = (
    "You are the Engineering Lead. You orchestrate Scout, Coder, and Sentinel "
    "to produce validated code.\n\n"
    "When you receive a user query, call orchestrate_build with that query exactly. "
    "Do not modify or summarize the query. Return the full result from orchestrate_build "
    "to the user (the code plus any footer). Do not add your own commentary; the result "
    "already includes approval or validation status."
)

root_agent = LlmAgent(
    name="eng-lead",
    model="gemini-2.5-pro-preview-02-05",
    description="Engineering Lead — Orchestrator for Scout, Coder, Sentinel.",
...
try:
    ...
    def _llm_usage_callback(callback_context, llm_response):
        try:
            _emit_llm_usage(
                llm_response,
                agent_id="agentic_lens_eng_lead",
