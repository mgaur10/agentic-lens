...
def _safe_load_root_agent() -> LlmAgent:
    ...
    # Filter to only allowed keys if we know them
    if _ALLOWED_KEYS:
        sanitized = {k: v for k, v in sanitized.items() if k in _ALLOWED_KEYS}
    return LlmAgent(
        name=sanitized.pop("name", "agentic_prism_chat"),
        model=sanitized.pop("model", DEFAULT_MODEL),
        description=sanitized.pop("description", "Chat agent"),
        instruction=sanitized.pop("instruction", ""),
        **{k: v for k, v in sanitized.items() if k not in ("name", "model", "description", "instruction")},
    )
